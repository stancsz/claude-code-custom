    ...
    if isinstance(value, list):
        ...
        for item in value:
            ...
            if item_dict:
                ...
                text_parts.append(json_string(item_dict))
                continue
...
def normalize_tool_call(tool_call):
    normalized_tool_call = as_plain_dict(tool_call)
    if not normalized_tool_call:
        return None

    function_def = as_plain_dict(normalized_tool_call.get("function")) or {}
...
def normalize_function_tool(tool):
    normalized_tool = as_plain_dict(tool)
    if not normalized_tool:
        return None

    function_def = as_plain_dict(normalized_tool.get("function"))
    ...
    normalized_function = as_plain_dict(normalized_tool.get("function"))
...
def normalize_tool_choice(tool_choice):
    normalized_choice = as_plain_dict(tool_choice)
    ...
    function_def = as_plain_dict(normalized_choice.get("function"))
...
def normalize_messages(messages):
    ...
    for message in messages:
        message_dict = as_plain_dict(message)
        if not message_dict:
        ...
        if not isinstance(content, list):
            if not isinstance(content, str):
                message_dict["content"] = stringify_content(content)
        ...
        if role == "assistant":
            ...
            for block in content:
                block_dict = as_plain_dict(block)
                if not block_dict:
                ...
                if block_type == "tool_use":
                    normalized_tool_call = normalize_tool_call(block_dict)
                    if normalized_tool_call:
                        tool_calls.append(normalized_tool_call)
                    continue

                fallback_text = stringify_content(block_dict)
                if fallback_text:
                    ...
                    normalized_messages.append(
                        {
                            ...
                            "content": stringify_content(block_dict.get("content")),
                        }
                    )
                    continue
...
def looks_like_anthropic_request(data):
    tools = data.get("tools")
    if isinstance(tools, list):
        for tool in tools:
            tool_dict = as_plain_dict(tool)
            if not tool_dict:
                continue
...
def inject_tool_argument_guardrail(messages):
    if not isinstance(messages, list):
        return messages
